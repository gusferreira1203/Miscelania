...
hasmatplot = 'y' == input("Este programa utiliza da biblioteca matplotlib. Digite 'y' se a possui. Caso contrário, funcionalidades dependentes não serão executadas. > ").lower()
if hasmatplot:
...
class Game:
    def __init__(self, x = 4, printgame = True, solve = True, fair = False, shuffle = 1000, worst = False):
        self.printgame = printgame
        self.x = x

        self.counter = 0
        self.kup, self.kdown, self.kleft, self.kright = ["w","^[[A"], ["s","^[[B"], ["a", "^[[D"], ["d","^[[c"]
   ...
   ////  ////////////////////////// ///// 
    //////// ///////////////// ///////// 
      //////  ///////////////   ////// 
            ...
            ///   w(·o·)w   /// 
            ///             ///""") 
                print("            ///" + " " * (6 - len(str(self.x))) + str(self.x) + "x" + str(self.x) + " " * (6 - len(str(self.x))) + "///")
                print("""            ///             /// 
            /////////////////// 
            /////////////////// 
          /////////////////////// 
...
class Interface:
    def __init__(self):
        ...
        while onoff:            
            ...
            if ABC.lower() == "a":
                a = Game(x = int(input("\nEscolha o tamanho do Tabuleiro (default: 4, mínimo: 2):\n >>> ")), printgame = True, solve = False, fair = True, shuffle = int(input("\nEscolha a quantidade de embaralhamentos (default: 1000):\n >>> ")))
            elif ABC.lower() == "b":
                shuffle = input("Digite um número para Embaralhar (Apenas jogos possíveis; Quanto maior o número, mais embaralhado, default: 1000) ou pressione 'Enter' para Randomizar (jogos possíveis e impossíveis) \n(Ou 'y' para um tabuleiro do tipo 'Pior cenário', que necessita do máximo número de lances) (!!Bastante pesado!!)\n\n> ")
                if len(shuffle) > 0 and type(shuffle) == "int":
                    a = Game(x = int(input("\nEscolha o tamanho do Tabuleiro (default: 4, mínimo: 2):\n >>> ")), printgame = True, solve = True, fair = True, shuffle = int(shuffle))
                elif shuffle.lower() == "y":
                    a = Game(x = int(input("\nEscolha o tamanho do Tabuleiro (default: 4, mínimo: 2):\n >>> ")), printgame = True, solve = True, worst = True)
                else:
                    a = Game(x = int(input("\nEscolha o tamanho do Tabuleiro (default: 4, mínimo: 2):\n >>> ")), printgame = True, solve = True, fair = False)
            elif ABC.lower() == "c":
                grafico = input("\n\nEscolha um gráfico:\n\n  A. Movimentos\n  B. Tempo\n\n >>> ")
                ...
                if grafico.lower() == "a":
                    if prova:
                        ...
                        self.title = "Relação Movimentos por Tamanho com precisão de " + str(m) + " repetições\n(com f(x+1)/f(x) tendendo a 1) "
                    else:
                    ...
                    if prova:
                        ...
                        self.title = "Relação Tempo por Tamanho com precisão de " + str(m) + " repetições\n(com f(x+1)/f(x) tendendo a 1) "
                    else:
                        self.label_y = "Tempo"
                        self.title = "Relação Tempo por Tamanho com precisão de " + str(m) + " repetições"
                first = int(input("Tamanho inicial (mínimo e recomendado: 3) > "))
                last = int(input("Tamanho final (recomendado 20 para execução regular e 12 para comparação com pior cenário) >")) + 1
                R = range(first, last)
                for i in R:
                    numero = 0
                    for j in range(m):
                ...
                if prova:
                    ...
                    for i in range(1, len(L)):
                        L2.append(L[i] / L[i-1])
                    L = L2
                elif 'y' == input("Comparar com um tabuleiro do tipo 'Pior cenário', que necessita do máximo número de lances? ('y' para aceitar)\n (!!Bastante pesado!!)\n\n >>> ").lower():
                    L2 = []
                    for i in R:
        ...
        """)
            ...
            onoff = input("Pressione 'Enter' para seguir e 'x' para encerrar > ").lower() != "x"
